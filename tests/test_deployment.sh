...
sed -i 's|ghcr.io/maksim87de/neyra-business-agent-network:v0.1.0|registry.example/neyra@sha256:test|' "$TMP/etc/deploy.env"
sed -i "s|^NEYRA_UID=.*|NEYRA_UID=$(id -u)|; s|^NEYRA_GID=.*|NEYRA_GID=$(id -g)|" "$TMP/etc/deploy.env"
