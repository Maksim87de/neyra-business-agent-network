...
_NEYRA_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (NEYRA_KANBAN_TASK env set) or the current
...
# Нэйра сохраняет анализ изображений, но не получает отдельный инструмент
# генерации картинок. Провайдеры изображений не должны включаться неявно.
_NEYRA_CORE_TOOLS = [
    tool for tool in _NEYRA_CORE_TOOLS if tool != "image_generate"
]

# Webhook events may originate from untrusted third-party content (for example,
# public PR titles/comments). Keep the default webhook toolset intentionally
# constrained to avoid local file/system execution by prompt injection.
_NEYRA_WEBHOOK_SAFE_TOOLS = [
    "web_search",
    "web_extract",
    "vision_analyze",
    "clarify",
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `neyra tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (NEYRA_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Neyra toolsets (CLI + messaging platforms)
    #
    ...
    "neyra-cli": {
        ...
        "tools": _NEYRA_CORE_TOOLS,
        "includes": []
    },

    "neyra-cron": {
        # Mirrors neyra-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `neyra tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as neyra-cli; gated by `neyra tools`",
        "tools": _NEYRA_CORE_TOOLS,
        "includes": []
    },

    "neyra-telegram": {
        "description": "Нэйра в Telegram: полный операторский набор без отдельного генератора изображений",
        "tools": _NEYRA_CORE_TOOLS,
        "includes": []
    },
    ...
    "neyra-email": {
        "description": "Email bot toolset - interact with Neyra via email (IMAP/SMTP)",
        "tools": _NEYRA_CORE_TOOLS,
        "includes": []
    },
    ...
    "neyra-sms": {
        "description": "SMS bot toolset - interact with Neyra via SMS (Twilio)",
        "tools": _NEYRA_CORE_TOOLS,
        "includes": []
    },
    ...
    "neyra-webhook": {
        ...
        "tools": _NEYRA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "neyra-gateway": {
        ...
        "tools": [],
        "includes": ["neyra-telegram", "neyra-discord", "neyra-whatsapp", "neyra-slack", "neyra-signal", "neyra-bluebubbles", "neyra-homeassistant", "neyra-email", "neyra-sms", "neyra-mattermost", "neyra-matrix", "neyra-dingtalk", "neyra-feishu", "neyra-wecom", "neyra-wecom-callback", "neyra-weixin", "neyra-qqbot", "neyra-webhook", "neyra-yuanbao"]
    }
}


_NEYRA_TOOLSET_ALIASES = {
    "neyra-cli": "neyra-cli",
    "neyra-cron": "neyra-cron",
    "neyra-discord": "neyra-discord",
    "neyra-whatsapp": "neyra-whatsapp",
    "neyra-slack": "neyra-slack",
    "neyra-signal": "neyra-signal",
    "neyra-bluebubbles": "neyra-bluebubbles",
    "neyra-homeassistant": "neyra-homeassistant",
    "neyra-email": "neyra-email",
    "neyra-mattermost": "neyra-mattermost",
    "neyra-matrix": "neyra-matrix",
    "neyra-dingtalk": "neyra-dingtalk",
    "neyra-feishu": "neyra-feishu",
    "neyra-wecom": "neyra-wecom",
    "neyra-wecom-callback": "neyra-wecom-callback",
    "neyra-weixin": "neyra-weixin",
    "neyra-qqbot": "neyra-qqbot",
    "neyra-webhook": "neyra-webhook",
    "neyra-yuanbao": "neyra-yuanbao",
    "neyra-sms": "neyra-sms",
    "neyra-gateway": "neyra-gateway",
}

for _neyra_name, _neyra_name in _NEYRA_TOOLSET_ALIASES.items():
    if _neyra_name in TOOLSETS and _neyra_name not in TOOLSETS:
        TOOLSETS[_neyra_name] = {
            "description": (
                f"Neyra compatibility alias for `{_neyra_name}`. "
                "Keeps rebranded configs on the upstream platform tool contract."
            ),
            "tools": [],
            "includes": [_neyra_name],
        }
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (neyra-<name>).
        # Gives them _NEYRA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("neyra-"):
            platform_name = name[len("neyra-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_NEYRA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
