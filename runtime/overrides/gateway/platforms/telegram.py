...
_LOCAL_BOT_API_DEFAULT_ROOT = "/opt/data/tg-bot-api"
_LOCAL_FILE_COPY_BUFFER_BYTES = 8 * 1024 * 1024


def _cache_local_bot_api_file(
    file_path: str,
    *,
    cache_dir: _Path,
    prefix: str,
    ...
    """Stream a file exposed by a local Bot API server into the profile cache.

    A local telegram-bot-api instance returns an absolute path from ``getFile``.
    Copying it through ``download_as_bytearray`` defeats that feature and makes a
    multi-gigabyte video consume the gateway's RAM.  Keep the source constrained
    to the explicitly shared Bot API directory, then use an atomic, bounded-RAM
    copy so the gateway owns a stable cache file after Bot API cleanup.
    """
    source = _Path(file_path).resolve(strict=True)
    allowed_root = _Path(local_files_root).resolve(strict=True)
    if not source.is_relative_to(allowed_root):
        raise ValueError("Local Bot API returned a path outside the shared media directory")
    if not source.is_file():
        raise ValueError("Local Bot API file path is not a regular file")

    cache_dir.mkdir(parents=True, exist_ok=True)
    final_path = cache_dir / f"{prefix}_{uuid.uuid4().hex[:12]}{suffix}"
    fd, temp_name = tempfile.mkstemp(prefix=f".{prefix}-", suffix=".part", dir=cache_dir)
    try:
        with os.fdopen(fd, "wb") as destination, source.open("rb") as source_file:
            shutil.copyfileobj(source_file, destination, length=_LOCAL_FILE_COPY_BUFFER_BYTES)
            destination.flush()
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("NEYRA_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "NEYRA_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "NEYRA_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
        ...
        # Public Bot API downloads are limited to 20 MB.  Our local Bot API
        # server is explicitly configured with --local, which supports
        # unlimited downloads; cap the gateway policy at 5 GiB so a malformed
        # update cannot fill the host disk. Telegram may still apply a lower
        # sender-side limit.
        try:
            configured_max = int(self.config.extra.get("max_incoming_file_bytes", 0))
        except (TypeError, ValueError):
            configured_max = 0
        self._max_doc_bytes: int = (
            configured_max or 5 * 1024 * 1024 * 1024
            if self.config.extra.get("local_mode") and self.config.extra.get("base_url")
            else 20 * 1024 * 1024
        )
        # State for interactive Telegram controls belongs to the adapter
        # lifecycle, not after an early return in a media helper.
    ...
    async def _cache_telegram_video_file(self, file_obj: Any, ext: str) -> str:
        """Cache a Telegram video without loading local-Bot-API files into RAM."""
        local_path = getattr(file_obj, "file_path", None)
        if self.config.extra.get("local_mode") and local_path:
            return _cache_local_bot_api_file(
                local_path,
                cache_dir=get_video_cache_dir(),
                prefix="video",
                suffix=ext,
                local_files_root=str(self.config.extra.get("local_files_root", _LOCAL_BOT_API_DEFAULT_ROOT)),
            )
        video_bytes = await file_obj.download_as_bytearray()
        return cache_video_from_bytes(bytes(video_bytes), ext=ext)
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Neyra-created private-chat topic lanes are marked with
    ...
    def _should_attempt_rich(self, content: str) -> bool:
        ...
        # __init__), so the flags may be unset — default rich ON (the
        # feature is opt-out via platforms.telegram.extra.rich_messages).
        return bool(
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `neyra update` or `--replace` handoffs)
        ...
        message = (
            "Не удалось восстановить получение сообщений Telegram после %d попыток (%ds ожидания). "
            "Предыдущий сеанс шлюза всё ещё удерживается серверами Telegram "
            "либо другой процесс использует тот же токен бота. "
            "Убедитесь, что другой экземпляр Нэйры с этим токеном не запущен, "
            "затем перезапустите шлюз командой `neyra gateway restart`."
    ...
    def _persist_dm_topic_thread_id(
        ...
        try:
            ...
            config_path = get_neyra_home() / "config.yaml"
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the Neyra process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("NEYRA_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("NEYRA_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("NEYRA_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("NEYRA_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("NEYRA_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("NEYRA_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            if webhook_url:
                ...
                if not webhook_secret:
                    raise RuntimeError(
                        ...
                        "security advisory GHSA-3vpc-7q5r-276h.\n\n"
            ...
            try:
                ...
                from neyra_cli.client_business_commands import telegram_client_menu_commands
                menu_commands = telegram_client_menu_commands()
                ...
                # A previously configured per-chat scope overrides private/default
                # scopes. Refresh every numeric DM allowlist entry so stale menus
                # cannot survive an otherwise successful deployment.
                for raw_chat_id in os.getenv("TELEGRAM_ALLOWED_USERS", "").split(","):
                    normalized_chat_id = raw_chat_id.strip()
                    if not normalized_chat_id.lstrip("-").isdigit():
                        continue
                    try:
                        await self._bot.set_my_commands(
                            bot_commands,
                            scope=BotCommandScopeChat(chat_id=int(normalized_chat_id)),
                        )
                    except Exception as scope_err:
                        logger.warning(
                            "[%s] set_my_commands FAILED for an allowed private chat: %s",
                            self.name,
                            scope_err,
                        )
                ...
                logger.info(
                    "[%s] Official client Telegram menu registered: %d commands",
                    self.name,
                    len(menu_commands),
                )
            except Exception as e:
    ...
    async def send(
        ...
        if not self._bot:
            return SendResult(success=False, error="Нет подключения")
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``neyra update --gateway``
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            text = (
                f"⚠️ <b>Требуется подтверждение команды</b>\n\n"
                f"<pre>{_html.escape(cmd_preview)}</pre>\n\n"
                f"Причина: {_html.escape(description)}"
            )
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Разрешить один раз", callback_data=f"ea:once:{approval_id}"),
                    InlineKeyboardButton("✅ На эту сессию", callback_data=f"ea:session:{approval_id}"),
                ],
                [
                    InlineKeyboardButton("✅ Разрешать всегда", callback_data=f"ea:always:{approval_id}"),
                    InlineKeyboardButton("❌ Отклонить", callback_data=f"ea:deny:{approval_id}"),
                ],
            ])
    ...
    async def send_slash_confirm(
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton(t("gateway.destructive_slash.button_once"), callback_data=f"sc:once:{confirm_id}"),
                    InlineKeyboardButton(t("gateway.destructive_slash.button_always"), callback_data=f"sc:always:{confirm_id}"),
                ],
                [
                    InlineKeyboardButton(t("gateway.destructive_slash.button_cancel"), callback_data=f"sc:cancel:{confirm_id}"),
                ],
            ])
    ...
    async def send_model_picker(
        ...
        try:
            # The official client catalog has a single provider. In that case
            # skip the redundant provider page and show model buttons directly.
            direct_provider = providers[0] if len(providers) == 1 else None
            if direct_provider is not None:
                direct_models = list(direct_provider.get("models", []))
                keyboard, page_info = self._build_model_keyboard(direct_models, 0)
                text = self.format_message(
                    (
                        f"⚙ *Выбор модели*\n\n"
                        f"Текущая модель: `{current_model or 'неизвестна'}`\n"
                        f"Выберите модель{page_info}:"
                    )
                )
            else:
            ...
            if direct_provider is not None:
                picker_state.update({
                    "selected_provider": direct_provider["slug"],
                    "selected_provider_name": direct_provider.get("name", direct_provider["slug"]),
                    "model_list": direct_models,
                    "model_page": 0,
                })
            self._model_picker_state[str(chat_id)] = picker_state
    ...
    def _build_model_keyboard(self, models: list, page: int) -> tuple:
        ...
        try:
            ...
            client_labels = dict(CLIENT_MODEL_CHOICES)
        except Exception:
            client_labels = {}

        for i, model_id in enumerate(page_models):
            abs_idx = start + i
            short = client_labels.get(
                model_id,
        ...
        rows.append([
            InlineKeyboardButton("◀ К провайдерам", callback_data="mb"),
    ...
    async def _handle_model_picker_callback(
        ...
        if not state:
            await query.answer(text="Меню устарело — снова вызовите /model.")
            return
        ...
        if data.startswith("mp:"):
            ...
            shown = len(models)
            extra = f"\n_Доступно ещё: {total - shown}. Можно сразу ввести `/model <имя>`_" if total > shown else ""

            await query.edit_message_text(
                text=self.format_message(
                    (
                        f"⚙ *Настройка модели*\n\n"
                        f"Провайдер: *{pname}*{page_info}\n"
                        f"Выберите модель:{extra}"
                    )
                ),
                parse_mode=ParseMode.MARKDOWN_V2,
                reply_markup=keyboard,
            ...
            if not callback:
                await query.answer(text="Меню устарело.")
                return
            ...
            try:
                ...
                result_text = f"Не удалось переключить модель: {exc}"
            ...
            try:
                ...
                try:
                    ...
                    pass
            await query.answer(text="Модель переключена!")
            ...
            await query.edit_message_text(
                text=self.format_message(
                    (
                        f"⚙ *Настройка модели*\n\n"
                        f"Текущая модель: `{state['current_model'] or 'неизвестна'}`\n"
                        f"Провайдер: {provider_label}\n\n"
                        f"Выберите провайдера:"
                    )
                ),
    ...
    async def _handle_callback_query(
        ...
        if data.startswith("ea:"):
            ...
            if len(parts) == 3:
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await query.answer(text="⛔ У вас нет права подтверждать команды.")
                    return

                session_key = self._approval_state.pop(approval_id, None)
                if not session_key:
                    await query.answer(text="Это подтверждение уже обработано.")
                    return

                # Map choice to human-readable label
                label_map = {
                    "once": "✅ Разрешено один раз",
                    "session": "✅ Разрешено на эту сессию",
                    "always": "✅ Разрешено постоянно",
                    "deny": "❌ Отклонено",
                }
        ...
        if data.startswith("sc:"):
            ...
            if len(parts) == 3:
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await query.answer(text=t("gateway.destructive_slash.callback_unauthorized"))
                    return

                session_key = self._slash_confirm_state.pop(confirm_id, None)
                if not session_key:
                    await query.answer(text=t("gateway.destructive_slash.callback_resolved"))
                    return

                label_map = {
                    "once": t("gateway.destructive_slash.callback_approved_once"),
                    "always": t("gateway.destructive_slash.callback_always"),
                    "cancel": t("gateway.destructive_slash.callback_cancelled"),
                }
                user_display = getattr(query.from_user, "first_name", "Пользователь")
                label = label_map.get(choice, t("gateway.destructive_slash.callback_resolved_label"))
                ...
                try:
                    await query.edit_message_text(
                        text=self.format_message(
                            t("gateway.destructive_slash.callback_by_user", label=label, user=user_display)
                        ),
                        parse_mode=ParseMode.MARKDOWN_V2,
                        reply_markup=None,
                    )
        ...
        if data.startswith("cl:"):
            ...
            if len(parts) == 3:
                ...
                if not self._is_callback_user_authorized(
                    ...
                    await query.answer(text="⛔ У вас нет права отвечать на этот запрос.")
                    return
                ...
                if choice_token == "other":
                    ...
                    await query.answer(text="✏️ Введите ответ в чате.")
                    try:
                        await query.edit_message_text(
                            text=f"❓ {query.message.text or ''}\n\n<i>Ожидается текстовый ответ от {_html.escape(user_display)}…</i>",
                            parse_mode=ParseMode.HTML,
                            reply_markup=None,
                        )
                    except Exception:
        ...
        if not self._is_callback_user_authorized(
            ...
            await query.answer(text="⛔ У вас нет права отвечать на запросы обновления.")
            return
        await query.answer(text="Ответ передан процессу обновления.")
        # Edit the message to show the choice and remove buttons
        label = "Да" if answer == "y" else "Нет"
        try:
            await query.edit_message_text(
    ...
    # Scripts live in ~/.neyra/scripts/gmail-triage/. `arg` from the callback
    ...
    _GT_VERB_DISPATCH = {
        ...
        "archive":      ("archive.sh",         [],         "✓ перемещено в архив",           False),
        "draft":        ("draft-blank.sh",     [],         "✓ ответ сохранён в черновик",      False),
        "spam":         ("spam.sh",            [],         "✓ помечено как спам",        False),
        "mute":         ("mute-add.sh",        ["email"],  "✓ отправитель скрыт",              True),
        "mute-domain":  ("mute-add.sh",        ["domain"], "✓ домен скрыт",       True),
        "trust":        ("trusted-ops-add.sh", ["email"],  "✓ отправитель доверенный",            True),
        "trust-domain": ("trusted-ops-add.sh", ["domain"], "✓ домен доверенный",     True),
        "vip":          ("vip-add.sh",         ["email"],  "✓ отправитель помечен VIP",         True),
        "vip-domain":   ("vip-add.sh",         ["domain"], "✓ домен помечен VIP",  True),
    }
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        parts = data.split(":", 2)
        if len(parts) != 3:
            await query.answer(text="Некорректные данные gmail-triage.")
            return
        ...
        if not self._is_callback_user_authorized(
            ...
            await query.answer(text="⛔ У вас нет права выполнять действия с этим письмом.")
            return
        ...
        script_path = _Path.home() / ".neyra" / "scripts" / "gmail-triage" / script_name
        if not script_path.exists():
            await query.answer(text=f"❌ Скрипт {script_name} не найден")
    ...
    def _telegram_observe_unmentioned_group_messages(self) -> bool:
        ...
        When enabled with ``require_mention``, Telegram matches the observe-first
        group UX: observe ordinary group chatter in the session
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@neyra_bot.example" are not mentions
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Neyra bot profiles. A message like
    ...
    async def _handle_media_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if msg.voice:
            ...
            try:
                if msg.video.file_size and msg.video.file_size > self._max_doc_bytes:
                    limit_gib = self._max_doc_bytes / (1024 * 1024 * 1024)
                    event.text = (
                        "Видео слишком большое или Telegram не передал его размер. "
                        f"Максимум для этого интерфейса: {limit_gib:g} GiB."
                    )
                    logger.info("[Telegram] Video exceeds inbound limit: %s bytes", msg.video.file_size)
                    await self.handle_message(event)
                    return
                ...
                if getattr(file_obj, "file_path", None):
                    for candidate in SUPPORTED_VIDEO_TYPES:
                        if file_obj.file_path.lower().endswith(candidate):
                            ext = candidate
                            break
                cached_path = await self._cache_telegram_video_file(file_obj, ext)
                event.media_urls = [cached_path]
        ...
        # Download document files to cache for agent processing
            ...
            try:
                ...
                if ext in SUPPORTED_VIDEO_TYPES:
                    if doc.file_size and doc.file_size > self._max_doc_bytes:
                        limit_gib = self._max_doc_bytes / (1024 * 1024 * 1024)
                        event.text = (
                            "Видео слишком большое или Telegram не передал его размер. "
                            f"Максимум для этого интерфейса: {limit_gib:g} GiB."
                        )
                        logger.info("[Telegram] Video document exceeds inbound limit: %s bytes", doc.file_size)
                        await self.handle_message(event)
                        return
                    file_obj = await doc.get_file()
                    cached_path = await self._cache_telegram_video_file(file_obj, ext)
