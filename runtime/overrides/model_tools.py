...
#   - cli.py, neyra_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from neyra_cli.config import get_config_path
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("NEYRA_KANBAN_TASK")),
        )
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("NEYRA_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by NEYRA_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like neyra-cli)
...
def handle_function_call(
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from neyra_cli.plugins import get_pre_tool_call_block_message
        ...
        try:
            from agent.runtime_observability import log_tool_call_event
            log_tool_call_event(
            ...
            logger.debug("tool-call telemetry error: %s", _telemetry_err)

        try:
            from neyra_cli.plugins import invoke_hook
            invoke_hook(
                "post_tool_call",
                tool_name=function_name,
                args=function_args,
        ...
        # Tool-result transformations are production allowlisted. Every attempted
        # transformation emits a content-safe append-only audit event with the
        # original result fingerprint and the responsible plugin name.
        original_result = result
        try:
            from agent.runtime_observability import log_tool_result_transform_event
            from neyra_cli.plugins import (
                get_transform_plugin_mode,
                invoke_hook_with_provenance,
            )
            hook_results = invoke_hook_with_provenance(
                ...
                tool_call_id=tool_call_id or "",
                duration_ms=duration_ms,
                original_result_hash=hashlib.sha256(original_result.encode("utf-8", errors="surrogatepass")).hexdigest(),
            )
            trusted_replacement: tuple[str, str] | None = None
            security_appends: list[tuple[str, str]] = []
            for plugin_name, hook_result in hook_results:
                if not isinstance(hook_result, str):
                    continue
                mode = get_transform_plugin_mode(plugin_name)
                if mode == "append-only":
                    security_appends.append((plugin_name, hook_result))
                    continue
                if mode != "replace":
                    log_tool_result_transform_event(
                        tool_name=function_name, plugin_name=plugin_name,
                        original_result=original_result, final_result=original_result,
                        applied=False, mode="deny", reason="plugin_not_allowlisted",
                        task_id=task_id or "", session_id=session_id or "", tool_call_id=tool_call_id or "",
                    )
                    continue
                if trusted_replacement is None:
                    trusted_replacement = (plugin_name, hook_result)
                else:
                    log_tool_result_transform_event(
                        tool_name=function_name, plugin_name=plugin_name,
                        original_result=original_result, final_result=original_result,
                        applied=False, mode="replace", reason="first_trusted_result_wins",
                        task_id=task_id or "", session_id=session_id or "", tool_call_id=tool_call_id or "",
                    )

            if trusted_replacement is not None:
                plugin_name, result = trusted_replacement
                log_tool_result_transform_event(
                    tool_name=function_name, plugin_name=plugin_name,
                    original_result=original_result, final_result=result,
                    applied=True, mode="replace", task_id=task_id or "",
                    session_id=session_id or "", tool_call_id=tool_call_id or "",
                )

            for plugin_name, appended_text in security_appends:
                separator = "\n" if result and appended_text else ""
                result = f"{result}{separator}{appended_text}"
                log_tool_result_transform_event(
                    tool_name=function_name, plugin_name=plugin_name,
                    original_result=original_result, final_result=result,
                    applied=True, mode="append-only", reason="security_plugin",
                    task_id=task_id or "", session_id=session_id or "", tool_call_id=tool_call_id or "",
                )
        except Exception as _hook_err:
            logger.debug("transform_tool_result hook/audit error: %s", _hook_err)
        ...
        if _dispatch_start is not None:
            try:
                ...
                logger.debug("failed tool-call telemetry error: %s", _telemetry_err)
