...
Copies bundled skills from the repo's skills/ directory into ~/.neyra/skills/
...
The manifest lives at ~/.neyra/skills/.bundled_manifest.
"""
...
from neyra_constants import get_bundled_skills_dir, get_neyra_home, get_optional_skills_dir
...
def _get_bundled_dir() -> Path:
    ...
    Checks NEYRA_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.neyra/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.neyra/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Bundled skills are the bootstrap and update set for the main Neyra home
    # only.  A profile under <root>/profiles/<name> owns its skill tree
    # explicitly; never seed or update it from the global bundle.
    #
    # This is path-based rather than marker-based so a profile remains stable
    # across restarts without hidden per-profile state.
    if NEYRA_HOME.parent.name == "profiles":
        return {
            "copied": [], "updated": [], "skipped": 0,
            "user_modified": [], "cleaned": [], "total_bundled": 0,
            "optional_provenance_backfilled": [],
            "bundled_sync_skipped": True,
            "reason": "bundled skills are synced only for the main Neyra home",
        }
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `neyra skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `neyra skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `neyra update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.neyra/skills/ ...")
