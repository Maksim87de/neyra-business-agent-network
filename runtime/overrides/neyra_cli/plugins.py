...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with neyra-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.neyra/plugins/<name>/``
3. **Project plugins** – ``./.neyra/plugins/<name>/`` (opt-in via
   ``NEYRA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``neyra_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
#
# Set ``NEYRA_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.neyra/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("NEYRA_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NEYRA_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Neyra loggers.
    """
    ...
    logger.debug(
        "NEYRA_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
ENTRY_POINTS_GROUP = "neyra_agent.plugins"

_NS_PARENT = "neyra_plugins"
...
class PluginManifest:
    ...
    #              in ~/.neyra/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``neyra plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``neyra honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``neyra <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`neyra_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from neyra_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``neyra model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: neyra_cli.main imports plugins indirectly
        from neyra_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_hook(self, hook_name: str, callback: Callable) -> None:
        ...
        self._manager._hook_plugin_names[id(callback)] = self.manifest.key or self.manifest.name
    ...
    def register_skill(
        ...
        ``~/.neyra/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        self._hook_plugin_names: Dict[int, str] = {}
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to neyra_cli/. Two layouts are
        ...
        # 2. User plugins (~/.neyra/plugins/)
        user_dir = get_neyra_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.neyra/plugins/)
        if _env_enabled("NEYRA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".neyra" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set NEYRA_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with neyra and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Neyra ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `neyra plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``neyra_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``neyra_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        return [
            result
            for _plugin_name, result in self.invoke_hook_with_provenance(
                hook_name, **kwargs
            )
        ]

    def invoke_hook_with_provenance(
        self, hook_name: str, **kwargs: Any
    ) -> List[tuple[str, Any]]:
        """Invoke a hook and retain the exact plugin identity for each result."""
        ...
        for cb in callbacks:
            try:
                ...
                if ret is not None:
                    results.append(
                        (self._hook_plugin_names.get(id(cb), "unknown"), ret)
                    )
            except Exception as exc:
                logger.warning(
    ...
    def hook_plugin_names(self, hook_name: str) -> List[str]:
        """Return callback provenance in invocation order without executing hooks."""
        return [
            self._hook_plugin_names.get(id(callback), "unknown")
...
def invoke_hook_with_provenance(
    ...
    """Invoke a lifecycle hook and pair every result with its plugin name."""
    return get_plugin_manager().invoke_hook_with_provenance(hook_name, **kwargs)


def get_hook_plugin_names(hook_name: str) -> List[str]:
    """Return provenance names for callbacks registered to *hook_name*."""
    return get_plugin_manager().hook_plugin_names(hook_name)


def is_trusted_transform_plugin(plugin_name: str) -> bool:
    """Allow only explicitly trusted, non-security plugins to replace tool output."""
    return get_transform_plugin_mode(plugin_name) == "replace"


def get_transform_plugin_mode(plugin_name: str) -> str:
    """Return ``replace``, ``append-only``, or ``deny`` for a transformer."""
    try:
        from neyra_cli.config import load_config
        config = load_config()
        security = cfg_get(config, "plugins", "security", default=[])
        trusted = cfg_get(config, "plugins", "trusted_transformers", default=[])
        if not isinstance(plugin_name, str):
            return "deny"
        security_names = set(security if isinstance(security, list) else [])
        trusted_names = set(trusted if isinstance(trusted, list) else [])
        if plugin_name in security_names:
            return "append-only"
        if plugin_name in trusted_names:
            return "replace"
        return "deny"
    except Exception:
        return "deny"
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="neyra-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``neyra tools`` TUI so plugin-provided toolsets appear
