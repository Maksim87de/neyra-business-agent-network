"""
Neyra Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m neyra_cli.main web          # Start on http://127.0.0.1:9119
    python -m neyra_cli.main web --port 8080
"""
...
from neyra_cli import __version__, __release_date__
from neyra_cli.config import (
    cfg_get,
    DEFAULT_CONFIG,
    OPTIONAL_ENV_VARS,
    get_config_path,
...
try:
    ...
    # running `neyra dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["NEYRA_WEB_DIST"]) if "NEYRA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(
    title="Нэйра — API",
    description="Технический API веб-интерфейса Нэйры.",
    version=__version__,
    docs_url="/api/docs",
    redoc_url="/api/redoc",
...
_SESSION_HEADER_NAME = "X-Neyra-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``neyra dashboard --tui``
# or NEYRA_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from neyra_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # and which providers are registered so ``neyra status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from neyra_cli.dashboard_auth import list_providers as _list_providers
...
# streamed to a per-action log file under ``~/.neyra/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_neyra_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "neyra-update": "neyra-update.log",
}
...
def _spawn_neyra_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``neyra <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``neyra_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "neyra_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "NEYRA_NONINTERACTIVE": "1"},
    }
...
# --- Neyra bubble-chat overlay: OpenAI-compatible chat proxy ----------------
...
def _read_dashboard_bubble_chat_flag() -> bool:
    """True when the dashboard should expose the Neyra bubble chat.

    Read from process env, then ``{NEYRA_HOME}/.env`` (mounted), so the flag can
    ...
    val = os.environ.get("NEYRA_DASHBOARD_CHAT", "")
    if val:
        return val == "1" or val.lower() == "true"
    env_path = Path(os.environ.get("NEYRA_HOME", "/opt/data")) / ".env"
    if env_path.exists():
        try:
            for raw in env_path.read_text().splitlines():
                ...
                if key.strip() != "NEYRA_DASHBOARD_CHAT":
                    continue
...
async def chat_completions_proxy(request: Request) -> Response:
    """Forward an OpenAI-compatible chat request to the Neyra gateway API server.
    ...
    the bubble-chat frontend (web/src/lib/sse-parser.ts) renders live
    tool-progress from ``event: neyra.tool.progress`` blocks carrying a
    {tool, toolCallId, status} payload, interleaved with the standard
    OpenAI ``chat.completion.chunk`` ``data:`` blocks. This proxy is a RAW
    passthrough (it forwards upstream SSE lines verbatim), so the contract
    lives upstream: the Neyra API_SERVER must emit that event. A plain
    OpenAI-compatible server only streams content deltas, so tool-progress
    stays silent — sse-parser ignores unknown events, chunks still render.
    Porting the API_SERVER (runtime contract) MUST preserve this emit.
    """
    ...
    for h in ("X-Neyra-Session-Id", "X-Neyra-Session-Key"):
...
async def chat_stop_proxy(request: Request) -> Response:
    """Interrupt the active dashboard chat task for one session.

    This is a control-plane request, separate from chat history: `/stop`
    must never be forwarded to the model as an ordinary user message.
    """
    import httpx as _httpx
    ...
    if not _CHAT_STOP_SAFE_ID.fullmatch(session_id):
        raise HTTPException(status_code=400, detail="Invalid session ID")

    upstream_url = (
        f"{_API_SERVER_PROXY_TARGET}/v1/chat/sessions/"
        f"{urllib.parse.quote(session_id, safe='')}/stop"
    )
    try:
        ...
        _log.error("chat_stop_proxy upstream error: %s", exc)
        raise HTTPException(status_code=502, detail="Unable to stop the active chat task")
...
# --- end Neyra bubble-chat overlay ------------------------------------------


# --- Client chat attachments ------------------------------------------------

_CHAT_UPLOAD_ROOT_NAME = "chat_uploads"
_CHAT_STOP_SAFE_ID = re.compile(r"^[A-Za-z0-9_-]{8,128}$")
_CHAT_UPLOAD_MAX_BYTES = 100 * 1024 * 1024
_CHAT_UPLOAD_CHUNK_BYTES = 1024 * 1024
_CHAT_UPLOAD_ALLOWED_EXTENSIONS = frozenset({
    ".aac", ".csv", ".doc", ".docx", ".flac", ".gif", ".jpeg", ".jpg",
    ".json", ".m4a", ".md", ".mp3", ".mp4", ".odt", ".ods", ".ogg",
    ".pdf", ".png", ".ppt", ".pptx", ".rtf", ".txt", ".wav", ".webm",
    ".webp", ".xls", ".xlsx", ".xml",
})
_CHAT_AUDIO_EXTENSIONS = frozenset({
    ".aac", ".flac", ".m4a", ".mp3", ".mp4", ".ogg", ".wav", ".webm",
})
_CHAT_SAFE_ID = re.compile(r"^[A-Za-z0-9_-]{8,128}$")


def _chat_upload_root() -> Path:
    root = get_neyra_home() / _CHAT_UPLOAD_ROOT_NAME
    root.mkdir(mode=0o700, parents=True, exist_ok=True)
    return root.resolve()


def _chat_safe_id(value: str, *, label: str) -> str:
    normalized = str(value or "").strip()
    if not _CHAT_SAFE_ID.fullmatch(normalized):
        raise HTTPException(status_code=400, detail=f"Invalid {label}")
    return normalized


def _chat_safe_filename(filename: str) -> str:
    raw = Path(str(filename or "attachment").replace("\x00", "")).name.strip()
    ...
    max_stem = max(1, 110 - len(suffix))
    return f"{stem[:max_stem]}{suffix}"


def _chat_session_upload_dir(session_id: str, *, create: bool = False) -> Path:
    sid = _chat_safe_id(session_id, label="session id")
...
def _chat_resolve_attachment(session_id: str, attachment_id: str) -> Path:
    aid = _chat_safe_id(attachment_id, label="attachment id")
    directory = _chat_session_upload_dir(session_id)
    if not directory.is_dir():
        raise HTTPException(status_code=404, detail="Attachment not found")
    matches = [
        item for item in directory.glob(f"{aid}--*")
...
def _chat_attachment_payload(
    ...
    *,
    ...
    stored_prefix = f"{attachment_id}--"
    display_name = path.name[len(stored_prefix):]
    ...
    return {
        ...
        "mime": mime_type,
        "kind": kind,
        # Both gateway and dashboard mount the same NEYRA_HOME at /opt/data.
        # The reference is intentionally explicit so the agent can inspect
        # documents with its normal file/vision tools without exposing a host
        # filesystem path.
        "agent_path": str(path),
    }
...
async def upload_chat_attachment(
    ...
    """Store one browser-chat attachment in the session-scoped workspace."""
    sid = _chat_safe_id(session_id, label="session id")
    safe_name = _chat_safe_filename(attachment.filename or "attachment")
    suffix = Path(safe_name).suffix.lower()
    if suffix not in _CHAT_UPLOAD_ALLOWED_EXTENSIONS:
        raise HTTPException(
            status_code=415,
            detail=f"Unsupported attachment type: {suffix or 'no extension'}",
        )

    normalized_kind = str(kind or "file").strip().lower()
    if normalized_kind not in {"file", "image", "audio", "voice"}:
        normalized_kind = "file"
    if normalized_kind in {"audio", "voice"} and suffix not in _CHAT_AUDIO_EXTENSIONS:
        raise HTTPException(status_code=415, detail="Voice attachment must be an audio file")

    directory = _chat_session_upload_dir(sid, create=True)
    attachment_id = secrets.token_hex(12)
    target = directory / f"{attachment_id}--{safe_name}"
    total = 0
    try:
        with target.open("xb") as output:
            os.chmod(target, 0o600)
            while True:
                chunk = await attachment.read(_CHAT_UPLOAD_CHUNK_BYTES)
                if not chunk:
                    break
                total += len(chunk)
    ...
    return _chat_attachment_payload(
        target,
        session_id=sid,
...
@app.get("/api/chat/attachments/{session_id}/{attachment_id}")
async def download_chat_attachment(session_id: str, attachment_id: str):
    path = _chat_resolve_attachment(session_id, attachment_id)
    display_name = path.name.split("--", 1)[1]
...
@app.delete("/api/chat/attachments/{session_id}/{attachment_id}")
async def delete_chat_attachment(session_id: str, attachment_id: str):
    path = _chat_resolve_attachment(session_id, attachment_id)
...
@app.post("/api/chat/attachments/{session_id}/{attachment_id}/transcribe")
async def transcribe_chat_attachment(session_id: str, attachment_id: str):
    """Transcribe a stored voice/audio attachment with Neyra's configured STT."""
    path = _chat_resolve_attachment(session_id, attachment_id)
    if path.suffix.lower() not in _CHAT_AUDIO_EXTENSIONS:
        raise HTTPException(status_code=415, detail="Attachment is not audio")

    try:
        from tools.transcription_tools import transcribe_audio
        result = await asyncio.to_thread(transcribe_audio, str(path))
    except Exception as exc:
        _log.exception("Browser voice transcription failed")
        raise HTTPException(status_code=500, detail="Voice transcription failed") from exc

    if not result.get("success"):
        _log.warning("Browser voice transcription rejected: %s", result.get("error", "unknown error"))
        raise HTTPException(status_code=422, detail="Не удалось распознать голосовое сообщение")
    transcript = str(result.get("transcript") or "").strip()
    if not transcript:
        raise HTTPException(status_code=422, detail="В голосовом сообщении не удалось распознать речь")
    return {
        "transcript": transcript,
        "provider": str(result.get("provider") or "configured"),
    }


# --- end client chat attachments -------------------------------------------


@app.post("/api/gateway/restart")
async def restart_gateway():
    """Kick off a ``neyra gateway restart`` in the background."""
    try:
        proc = _spawn_neyra_action(["gateway", "restart"], "gateway-restart")
    except Exception as exc:
...
@app.post("/api/neyra/update")
async def update_neyra():
    """Kick off ``neyra update`` in the background."""
    try:
        proc = _spawn_neyra_action(["update"], "neyra-update")
    except Exception as exc:
        _log.exception("Failed to spawn neyra update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Neyra supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in neyra_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.neyra/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``neyra auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Neyra resolves Anthropic creds in this order at runtime:
    1. ``~/.neyra/.anthropic_oauth.json`` — Neyra-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_neyra_oauth_credentials,
            read_claude_code_credentials,
            _NEYRA_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_neyra_oauth_credentials = None  # type: ignore
        _NEYRA_OAUTH_FILE = None  # type: ignore

    neyra_creds = None
    if read_neyra_oauth_credentials:
        try:
            neyra_creds = read_neyra_oauth_credentials()
        except Exception:
            neyra_creds = None
    if neyra_creds and neyra_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "neyra_pkce",
            "source_label": f"Neyra PKCE ({_NEYRA_OAUTH_FILE})",
            "token_preview": _truncate_token(neyra_creds.get("accessToken")),
            "expires_at": neyra_creds.get("expiresAt"),
            "has_refresh_token": bool(neyra_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Neyra even
    when they also have a separate Neyra-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "neyra auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "neyra auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "neyra auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "neyra auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "neyra auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from neyra_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("neyra_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Neyra-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _NEYRA_OAUTH_FILE
            if _NEYRA_OAUTH_FILE.exists():
                _NEYRA_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from neyra_cli.auth import clear_provider_auth
...
#          → persists to ~/.neyra/.anthropic_oauth.json AND credential pool
...
# Guarded so neyra web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Neyra file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``neyra auth add anthropic``.
    """
    from agent.anthropic_adapter import _NEYRA_OAUTH_FILE
    payload = {
    ...
    _NEYRA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _NEYRA_OAUTH_FILE.with_name(
        f"{_NEYRA_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _NEYRA_OAUTH_FILE)
        try:
            _NEYRA_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            "Content-Type": "application/json",
            "User-Agent": "neyra-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("NEYRA_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from neyra_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = (
            os.getenv("NEYRA_CODEX_BASE_URL", "").strip().rstrip("/")
...
async def delete_session_endpoint(session_id: str):
    ...
    # Browser uploads are session-owned data, not an independent archive.
    # Removing a chat therefore removes its attachments as well so client
    # capsules do not accumulate documents or voice recordings silently.
    try:
        upload_dir = _chat_session_upload_dir(session_id)
    except HTTPException:
        upload_dir = None
...
async def get_logs(
    ...
    from neyra_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_neyra_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from neyra_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from neyra_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, NEYRA_HOME)."""
    from neyra_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process NEYRA_HOME at import time. The dashboard is a single
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_neyra_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "neyra setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from neyra_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from neyra_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from neyra_cli.tools_config import (
...
#
# The endpoint spawns the same ``neyra --tui`` binary the CLI uses, behind
...
try:
    from neyra_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from neyra_cli.dashboard_auth.audit import AuditEvent, audit_log
        from neyra_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    Default: whatever ``neyra --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``NEYRA_TUI_RESUME`` env var —
    matching what ``neyra_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``NEYRA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from neyra_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("NEYRA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("NEYRA_TUI_INLINE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["NEYRA_TUI_RESUME"] = resume

    if sidecar_url:
        env["NEYRA_TUI_SIDECAR_URL"] = sidecar_url
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from neyra_cli.dashboard_auth.ws_tickets import mint_ticket
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Neyra inside WSL2 to use the dashboard's /chat "
...
# NEYRA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`neyra_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from neyra_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/neyra/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /neyra`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__NEYRA_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/neyra``)
        ...
        ``__NEYRA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__NEYRA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f"window.__NEYRA_DASHBOARD_CHAT__={bubble_js};"
                f'window.__NEYRA_BASE_PATH__="{prefix}";'
                f"window.__NEYRA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__NEYRA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__NEYRA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f"window.__NEYRA_DASHBOARD_CHAT__={bubble_js};"
                f'window.__NEYRA_BASE_PATH__="{prefix}";'
                f"window.__NEYRA_AUTH_REQUIRED__={gated_js};"
        ...
        if prefix:
            ...
            # Neyra overlay: our brand assets (favicon/wordmark) live under /brand/.
            html = html.replace('href="/brand/', f'href="{prefix}/brand/')
            html = html.replace('src="/brand/', f'src="{prefix}/brand/')
    ...
    # under ``/neyra`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Neyra backend. Intercept CSS asset
...
# Built-in dashboard color modes — label + description only.  The actual color
# definitions live in the frontend (web/src/themes/presets.ts).
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default", "label": "Dark", "description": "Neyra dark blue interface"},
    {"name": "light",   "label": "Light", "description": "Neyra light interface"},
]

_BUILTIN_DASHBOARD_THEME_NAMES = {t["name"] for t in _BUILTIN_DASHBOARD_THEMES}


def _normalise_dashboard_theme_name(name: Any) -> str:
    if isinstance(name, str) and name in _BUILTIN_DASHBOARD_THEME_NAMES:
        return name
    return "default"
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.neyra/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.neyra/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_neyra_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    """Return the available Neyra color modes and the currently active one.

    User-authored theme YAMLs are intentionally not exposed in the Neyra
    product shell; the dashboard presents a single brand with dark/light modes.
    """
    config = load_config()
    active = _normalise_dashboard_theme_name(
...
async def set_dashboard_theme(body: ThemeSetBody):
    """Set the active dashboard color mode (persists to config.yaml)."""
    config = load_config()
    if "dashboard" not in config:
        config["dashboard"] = {}
    theme = _normalise_dashboard_theme_name(body.name)
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as neyra_cli.plugins):
    1. User plugins:    ~/.neyra/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.neyra/plugins/  (only if NEYRA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from neyra_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_neyra_home() / "plugins", "user"),
    ...
    # ``neyra_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("NEYRA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".neyra" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from neyra_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_neyra_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"neyra auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from neyra_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from neyra_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from neyra_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from neyra_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.neyra/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"neyra_dashboard_plugin_{plugin['name']}"
...
from neyra_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from neyra_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing NEYRA_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Neyra Agent exposes a
    ...
    print(f"  Веб-интерфейс Нэйры → http://{host}:{port}")
