"""
Configuration management for Neyra.

Config files are stored in ~/.neyra/ for easy access:
- ~/.neyra/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.neyra/.env         - API keys and secrets

This module provides:
- neyra config           - Show current configuration
- neyra config edit      - Open config in editor
- neyra config set       - Set a specific value
- neyra config wizard    - Re-run setup wizard
"""
...
from neyra_cli.secret_prompt import masked_secret_prompt
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.neyra/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``neyra logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  neyra config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Neyra makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Neyra itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``neyra update``, the TUI build.
...
#   on every plugin install / ``neyra update``.
...
# * ``NEYRA_HOME`` / ``NEYRA_PROFILE`` / ``NEYRA_CONFIG`` /
#   ``NEYRA_ENV`` — Neyra runtime location flags. Writing these into
...
#
# IMPORTANT: ``NEYRA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (NEYRA_GEMINI_CLIENT_ID,
# NEYRA_LANGFUSE_PUBLIC_KEY, NEYRA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Neyra runtime location — never via dashboard env writer.
    # NOT a NEYRA_* blanket: integration credentials (NEYRA_GEMINI_*,
    # NEYRA_LANGFUSE_*, NEYRA_SPOTIFY_*, ...) ARE allowed.
    "NEYRA_HOME", "NEYRA_PROFILE", "NEYRA_CONFIG", "NEYRA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Neyra runtime location "
            "(NEYRA_HOME, NEYRA_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `neyra plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "NEYRA_LANGFUSE_ENV",
    "NEYRA_LANGFUSE_RELEASE",
    "NEYRA_LANGFUSE_SAMPLE_RATE",
    "NEYRA_LANGFUSE_MAX_CHARS",
    "NEYRA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from neyra_cli.colors import Colors, color
from neyra_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("NEYRA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_neyra_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Neyra is running in package-manager-managed mode.

    Two signals: the NEYRA_MANAGED env var (set by the systemd service),
    or a .managed marker file in NEYRA_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Neyra was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.neyra/.install_method`` file (written by installers)
    2. NEYRA_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_neyra_home() / ".install_method"
    try:
    ...
    from neyra_constants import is_container
    if is_container():
        return "docker"
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.neyra/.install_method."""
    stamp = get_neyra_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        ...
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade neyra-agent"
        return "pip install --upgrade neyra-agent"
    return "neyra update"
...
# Long-form text for ``neyra update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# neyra_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Neyra, it doesn't update
...
✗ ``neyra update`` не применяется внутри Docker container.

Neyra работает как container image, а не как git checkout: внутри контейнера
...
Примечания:
  • Если закреплён конкретный tag (например, ``:v0.14.0``), ``:latest`` не
    сдвинет контейнер. Забери нужный tag или переключись на ``:latest`` /
    ``:main`` для rolling updates.
  • Config и session history лежат под ``$NEYRA_HOME`` (в контейнере обычно
    ``/opt/data`` с bind mount с хоста) и сохраняются между обновлениями image.
  • Если это fork, собери свой image через ``Dockerfile`` и замени ``docker pull``
    на свой build/push pipeline."""


def format_docker_update_message() -> str:
    """Return the user-facing message for ``neyra update`` inside Docker.
...
def format_managed_message(action: str = "изменить установку Neyra") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("NEYRA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Нельзя выполнить действие ({action}): эта установка Neyra управляется NixOS "
            f"(NEYRA_MANAGED={env_hint}).\n"
            "Измени services.neyra-agent.settings в configuration.nix и выполни:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Нельзя выполнить действие ({action}): эта установка Neyra управляется Homebrew "
            f"(NEYRA_MANAGED={env_hint}).\n"
            "Используй:\n"
            "  brew upgrade neyra-agent"
        )

    return (
        f"Нельзя выполнить действие ({action}): эта установка Neyra управляется {managed_system}.\n"
        "Используй package manager, чтобы обновить или переустановить Neyra."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from NEYRA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, neyra_bin
    or None if container mode is not active, we're already inside the
    container, or NEYRA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("NEYRA_DEV") == "1":
        return None
    ...
    container_mode_file = get_neyra_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "neyra-agent")
    exec_user = info.get("exec_user", "neyra")
    neyra_bin = info.get("neyra_bin", "/data/current-package/bin/neyra")
...
# Re-export from neyra_constants — canonical definition lives there.
from neyra_constants import get_neyra_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_neyra_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_neyra_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the neyra group can
    share state with the gateway service.

    The mode can be overridden via the NEYRA_HOME_MODE environment variable
    (e.g. NEYRA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse NEYRA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("NEYRA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Neyra runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("NEYRA_CONTAINER") or os.environ.get("NEYRA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set NEYRA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into NEYRA_HOME if the user doesn't have one yet."""
...
def ensure_neyra_home():
    """Ensure ~/.neyra directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_neyra_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_neyra_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"NEYRA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Neyra-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Neyra auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Neyra sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Neyra
        ...
        #  "/home/user/.neyra/cache/documents:/output"]
        ...
        # bundled Neyra image, which drops to the `neyra` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Neyra sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Neyra should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``neyra chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.neyra/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: neyra sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Neyra truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``neyra kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``neyra kanban decompose`` and the kanban
        ...
        # ``neyra profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `neyra model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `neyra --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `neyra -c` muscle memory.  Default off so existing
        # users aren't surprised.  NEYRA_TUI_RESUME=<id> always wins.
        ...
        # Supported: en, ru, uk.  Unknown values fall back to en.
        "language": "ru",
        ...
        # e.g. `model · 68% · ~/projects/neyra`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``NEYRA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``NEYRA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # Public URL override (env: ``NEYRA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/neyra``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.neyra/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.neyra/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Neyra feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Neyra auto-pauses the goal and
    ...
    # always goes to ~/.neyra/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${NEYRA_SKILL_DIR} and ${NEYRA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `neyra curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `neyra curator run --consolidate` overrides
        # this for a single invocation.
        "consolidate": False,
        # Optional pre-run snapshot. Disabled by default: normal curator
        # runs must not create backup trees unless the operator explicitly
        # opts into this recovery mechanism.
        "backup": {
            "enabled": False,
    ...
    # This section is only needed for neyra-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "Neyra" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (NEYRA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.neyra/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or NEYRA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `neyra doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``neyra_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Neyra to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via NEYRA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `neyra -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `neyra` launches with
        ...
        # decomposition is manual via `neyra kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     neyra-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.neyra/logs/.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.neyra/.env,
        ...
        # behavior: files must live under the Neyra cache, under
        ...
        # user. Bridged to NEYRA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Neyra
        # cache (~/.neyra/cache/{documents,images,audio,video,screenshots})
        ...
        # to NEYRA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # NEYRA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to NEYRA_MEDIA_TRUST_RECENT_SECONDS.
        # Only consulted when ``strict`` is true.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.neyra/state.db.
    ...
    "sessions": {
        ...
        # default of ``neyra sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.neyra/sessions/session_{sid}.json`` on every turn
    ...
    # ``neyra update`` behaviour.
    "updates": {
        # Run a full ``neyra backup``-style zip of NEYRA_HOME before every
        # ``neyra update``.  Backups land in ``<NEYRA_HOME>/backups/`` and
        # can be restored with ``neyra import <path>``.  Off by default —
        # on large NEYRA_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<NEYRA_HOME>/lsp/bin/`` on first use.
    ...
    # `neyra tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.neyra/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.neyra/.env (or your shell) and never in config.yaml.
            ...
            # ~/.neyra/bin/ on first use.  When False you must install
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'neyra model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Neyra auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @neyra:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. NEYRA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #neyra)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: neyra-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'neyra-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Neyra API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Neyra API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Neyra API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # NEYRA_TOOL_PROGRESS and NEYRA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.neyra.config`` entries, then checks
    ...
    try:
        ...
        # should never break `neyra update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Neyra versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``neyra_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'neyra setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Neyra won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'neyra doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("NEYRA_HOME", "~/.neyra")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("NEYRA_TOOL_PROGRESS")
            old_mode = get_env_value("NEYRA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from NEYRA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from NEYRA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("NEYRA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from NEYRA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$NEYRA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_neyra_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `neyra plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `neyra curator status` has no
    ...
    #   3. Creates `~/.neyra/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_neyra_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_neyra_home() / "logs" / "curator"
    ...
    # ── Version 24 → 25: seed curator.consolidate (default false) ──
    ...
    # the curator is enabled; only the aux-model-cost LLM pass is gated. The
    # runtime deep-merge already supplies the default, but we seed the key so
    # it's visible/editable in config.yaml. Existing installs that WANT the old
    # always-consolidate behavior must set it to true explicitly. Only add the
    # key when a curator section exists and lacks it — never clobber a user value.
    if current_ver < 25:
        config = read_raw_config()
        raw_curator = config.get("curator")
        if isinstance(raw_curator, dict) and "consolidate" not in raw_curator:
            ...
            config["curator"] = raw_curator
            save_config(config)
            results["config_added"].append("curator.consolidate (default false)")
            if not quiet:
                print("  ✓ Seeded curator.consolidate=false in config.yaml")

    # ── Version 25 → 26: seed telegram.extra.rich_messages (default true) ──
    # Bot API 10.1 rich messages (sendRichMessage/sendRichMessageDraft) are
    # enabled by default. The runtime deep-merge already supplies the
    # default, but we seed the key so it's visible/editable in config.yaml.
    # Existing installs that want to opt out (e.g. a client that accepts but
    # cannot render rich messages) set it to false. Only add the key when a
    # telegram section exists — never clobber a user value.
    if current_ver < 26:
        config = read_raw_config()
        raw_telegram = config.get("telegram")
        if isinstance(raw_telegram, dict):
            raw_extra = raw_telegram.get("extra")
            if not isinstance(raw_extra, dict):
                raw_extra = {}
            if "rich_messages" not in raw_extra:
                raw_extra["rich_messages"] = True
                raw_telegram["extra"] = raw_extra
                config["telegram"] = raw_telegram
                save_config(config)
                results["config_added"].append("telegram.extra.rich_messages (default true)")
                if not quiet:
                    print("  ✓ Seeded telegram.extra.rich_messages=true in config.yaml")
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: neyra config set <key> <value>")
    ...
    # metadata.neyra.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _neyra_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.neyra/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.neyra/config.yaml.
    ...
    (which change ``NEYRA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — neyra auth) — OpenAI Codex
#   nous         (OAuth — neyra auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.neyra/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.neyra/.env.
    ...
    interactive menu render (`neyra tools`, `neyra setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `neyra tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Neyra env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.neyra/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.neyra/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.neyra/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.neyra/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Neyra — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Neyra vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.neyra/.env or environment."""
...
def show_config():
    ...
    print(color("│                 Neyra Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from neyra_cli.auth import get_anthropic_key
    ...
    print(color("  neyra config edit      # Edit config file", Colors.DIM))
    print(color("  neyra config set <key> <value>", Colors.DIM))
    print(color("  neyra setup            # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: neyra config set <key> <value>")
            print()
            print("Examples:")
            print("  neyra config set model anthropic/claude-sonnet-4")
            print("  neyra config set terminal.backend docker")
            print("  neyra config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'neyra config migrate' to add them")
        ...
        print("  neyra config            Show current configuration")
        print("  neyra config edit       Open config in editor")
        print("  neyra config set <key> <value>   Set a config value")
        print("  neyra config check      Check for missing/outdated config")
        print("  neyra config migrate    Update config with new options")
        print("  neyra config path       Show config file path")
        print("  neyra config env-path   Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``neyra config`` UI so users
