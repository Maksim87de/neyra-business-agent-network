...
``$NEYRA_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-neyra-setup
(the stage2 hook) has chowned the volume and seeded $NEYRA_HOME, but
...
# These prior states trigger automatic restart on container boot: they
# mean the gateway was alive when the container stopped, so it should
# come back up. ``draining`` is the transient state during a graceful
# shutdown/recreate — a SIGTERM'd container is killed mid-drain, before
# it can reach ``stopped`` or re-record ``running`` (running -> draining
# -> stopped). Without it, every profile gateway that was up stays down
# after a `docker compose up --force-recreate` (#orch-canary 2026-06-10).
# Everything else (startup_failed, starting, stopped, missing) registers
# the slot in the down state and waits for explicit user action — this
# avoids the crash-loop where a broken gateway keeps being restarted
# across `docker restart` cycles, and respects an intentional
# ``gateway stop`` (which records ``stopped``, not ``draining``).
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$NEYRA_HOME``,
    not under ``profiles/``). The dispatcher in ``neyra_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``neyra gateway start`` (no ``-p``) targets. Without it,
    bare ``neyra gateway start`` inside the container would land on
    ...
    ``$NEYRA_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        neyra_home: The container's NEYRA_HOME (typically /opt/data).
            Profiles live under ``<neyra_home>/profiles/<name>/``;
            the default profile lives at ``<neyra_home>`` itself.
    ...
    # ``neyra gateway start`` (no ``-p``) has somewhere to land;
    # auto-up only when the prior state was "running" (same rule as
    # named profiles).
    default_prior_state = _read_prior_state(neyra_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(neyra_home)
    ...
    profiles_root = neyra_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `neyra profile create` (config.yaml
            # is not — that comes later via `neyra setup`). Use it as the
            ...
            # via ``neyra -p default-named gateway start`` if they
            ...
            should_start = (
                ...
                or _profile_requests_autostart(entry)
            )
            ...
            if not dry_run:
                ...
                _register_service(
                    ...
                    entry.name,
                    start=should_start,
                    extra_env=_profile_gateway_env(entry),
                )
    ...
    if not dry_run:
        _write_reconcile_log(neyra_home, actions)
    return actions
...
def _profile_gateway_env(profile_dir: Path) -> dict[str, str]:
    """Return process-level overrides required by a profile gateway.

    The API-server switch is read from environment variables before the
    gateway loads profile configuration. Named profiles inherit the container
    environment, so they would otherwise all inherit the root API key and
    race to bind the root API port. Keep the API owned by the root Neyra
    gateway unless a named profile explicitly opts in to its own listener.

    This preserves an intentional specialist API endpoint while making the
    safe default for Telegram subagents (finance, legal, media, research) no
    API listener at all.
    """
    config_path = profile_dir / "config.yaml"
    if not config_path.exists():
        return {"API_SERVER_ENABLED": "false", "API_SERVER_KEY": ""}
    try:
        import yaml

        config = yaml.safe_load(config_path.read_text()) or {}
    except Exception:
        log.warning("could not read profile gateway config: %s", config_path)
        return {"API_SERVER_ENABLED": "false", "API_SERVER_KEY": ""}
    platforms = config.get("platforms")
    api_server = platforms.get("api_server") if isinstance(platforms, dict) else None
    enabled = api_server.get("enabled") if isinstance(api_server, dict) else False
...
def _profile_requests_autostart(profile_dir: Path) -> bool:
    """Return whether this named profile is explicitly always-on."""
    config_path = profile_dir / "config.yaml"
    if not config_path.exists():
        return False
    try:
    ...
    return bool(gateway.get("autostart")) if isinstance(gateway, dict) else False
...
def _register_service(
    ...
    *,
    ...
    from neyra_cli.service_manager import (
        S6ServiceManager,
    ...
    try:
        ...
        # stay consistent. Some gateway switches must be exported before
        # config.yaml is loaded; _profile_gateway_env supplies those values.
        run = tmp_dir / "run"
        run.write_text(
            S6ServiceManager._render_run_script(profile, extra_env=extra_env or {})
        )
        run.chmod(0o755)
        ...
        # it up explicitly with `neyra -p <profile> gateway start`
        # (which routes through the Phase 4
        # _dispatch_via_service_manager_if_s6 helper to `s6-svc -u`).
        if not start:
            (tmp_dir / "down").touch()

        # Pre-create the supervise/ skeleton with neyra ownership
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit neyra
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the neyra user) won't hit EACCES. See
...
def _write_reconcile_log(
    neyra_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $NEYRA_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    neyra_home = Path(os.environ.get("NEYRA_HOME", "/opt/data"))
    scandir = Path(os.environ.get("S6_PROFILE_GATEWAY_SCANDIR", "/run/service"))
    actions = reconcile_profile_gateways(
        neyra_home=neyra_home, scandir=scandir,
    )
    for a in actions:
        print(
