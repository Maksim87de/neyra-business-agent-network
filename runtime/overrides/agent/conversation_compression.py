    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `neyra setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Neyra "
...
def compress_context(
    ...
    *,
    ...
    log_prompt_event(
        "system_prompt.built",
        new_system_prompt,
        build_reason="context_compression",
    )
    ...
    if agent._session_db:
        try:
            ...
            try:
                ...
                os.environ["NEYRA_SESSION_ID"] = agent.session_id
            agent._session_db_created = False
            agent._session_db.create_session(
                session_id=agent.session_id,
                source=agent.platform or os.environ.get("NEYRA_SESSION_SOURCE", "cli"),
                model=agent.model,
            ...
            log_prompt_event(
                ...
                new_system_prompt,
                persist_reason="context_compression",
                session_id=agent.session_id,
            )
    ...
    # compression (not a fresh /new). Plugin engines (e.g. neyra-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See neyra-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    def _shrink_data_url(url: str) -> Optional[str]:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="neyra_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
