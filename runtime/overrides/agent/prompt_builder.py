...
from neyra_constants import get_neyra_home, get_skills_dir, is_wsl
...
# ---------------------------------------------------------------------------
# Context file scanning — detect prompt injection / promptware in AGENTS.md and
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "evidence. Если Юрист или Финансист завершился с ошибкой, заблокирован или "
    "не вернул evidence, Нэйра не называет его текст юридическим или финансовым "
    "заключением и прямо сообщает об отсутствии подтверждённого вывода. Telegram "
    "gateway остаётся единым центральным каналом Нэйры; "
...
NEYRA_AGENT_HELP_GUIDANCE = (
    ...
    "answer from the actual Neyra repo/docs instead of guessing. "
    "Docs: the live local Neyra source and operator-provided upstream documentation"
)
...
SESSION_SEARCH_GUIDANCE = (
    ...
    "relevant cross-session context exists, use recall_search first: it queries "
    "history, canonical knowledge, working plans, and files through one fast typed retrieval facade. Use "
    "session_search after a history hit when you need the wider message window or "
    "session lineage. If recall_search reports a partial backend failure, use the "
    "corresponding low-level tool only for that failed layer. Do not claim the "
    "artifact is missing until all applicable layers have been checked."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.neyra/kanban.db`. Your task id is in "
    "`$NEYRA_KANBAN_TASK`; your workspace is `$NEYRA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $NEYRA_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `neyra kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Neyra WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Neyra itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Neyra runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Neyra itself is running. The host OS, home, and cwd "
                f"of the Neyra process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Neyra "
...
_SKILLS_PROMPT_EXCLUDED_ROOT_CATEGORIES = frozenset({"donor-legacy"})


def _skills_prompt_snapshot_path() -> Path:
    return get_neyra_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.neyra/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("NEYRA_PLATFORM")
        or get_session_env("NEYRA_SESSION_PLATFORM")
        or ""
    )
    ...
    # Quarantined donor procedures stay on disk and remain explicitly loadable,
    # but they must not compete with central skills in the default system prompt.
    for category in list(skills_by_category):
        root_category = category.split("/", 1)[0]
        if root_category in _SKILLS_PROMPT_EXCLUDED_ROOT_CATEGORIES:
            skills_by_category.pop(category, None)

    if not skills_by_category:
        result = ""
    else:
        # Keep tier 0 intentionally tiny.  Names and descriptions belong to the
        # local retrieval tool, not to every turn's permanent system prompt.
        # This is the first part of progressive disclosure: category counts tell
        # the model that a capability exists, while skills_list(query=...) builds
        # a task-specific shortlist before any full SKILL.md is loaded.
        index_lines = []
        for category in sorted(skills_by_category.keys()):
            unique_count = len({name for name, _desc in skills_by_category[category]})
            cat_desc = category_descriptions.get(category, "")
            suffix = f" — {cat_desc}" if cat_desc else ""
            index_lines.append(f"  - {category} ({unique_count}){suffix}")

        result = (
            "## Skills (mandatory, lazy loading)\n"
            "Before a non-trivial task, classify it and call skills_list(query=<task keywords>, limit=5). "
            "Use its shortlist to select the single most specific relevant skill, then load only that "
            "skill with skill_view(name). Add at most one supporting skill, and only when it covers a "
            "distinct required layer that the primary skill does not. Do not load overlapping or loosely "
            "related skills. Proceed without a skill when none directly matches.\n"
            "Skills_list ranks local compact metadata and honours each skill's optional triggers, "
            "do_not_use_when rules, and priority. Treat a returned exclusion as decisive.\n"
            "Categories marked donor-legacy, archive, migration, or quarantine are excluded from ordinary "
            "work and may be selected only for an explicit audit, migration, or donor task.\n"
            "Whenever the user asks you to configure, set up, install, enable, disable, modify, or "
            "troubleshoot Neyra itself — its CLI, config, models, providers, tools, skills, voice, gateway, "
            "plugins, or any feature — use the actual Neyra docs, repo files, and available Neyra-related "
            "skills before answering. Prefer real commands (e.g. `neyra config set …`, `neyra tools`, "
            "`neyra setup`) instead of guessing or inventing workarounds.\n"
            "When a loaded skill materially affected the result, record a concise skill_feedback outcome "
            "after verification: good, partial, or bad. This records aggregate evidence only and never "
            "rewrites a skill automatically.\n"
            "If a skill has issues, fix it with skill_manage(action='patch'). After difficult/iterative tasks, "
            "offer to save the proven approach as a skill.\n\n"
            "<skill_category_index>\n"
            + "\n".join(index_lines) + "\n"
            "</skill_category_index>\n\n"
            "Prefer precision over skill count: one primary skill, optionally one supporting skill."
        )
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    try:
        from neyra_cli.nous_subscription import get_nous_subscription_features
    ...
    lines.extend(
        [
            ...
            "Useful commands: neyra setup, neyra setup tools, neyra setup terminal, neyra status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from NEYRA_HOME and return its content, or None.
    ...
    """
    try:
        from neyra_cli.config import ensure_neyra_home
        ensure_neyra_home()
    except Exception as e:
        logger.debug("Could not ensure NEYRA_HOME before loading SOUL.md: %s", e)

    soul_path = get_neyra_home() / "SOUL.md"
...
def build_context_files_prompt(
    ...
    """Load Neyra's central core plus an optional project ``AGENTS.md``.

    ``AGENTS.md`` from the active Neyra home is the permanent operator core.
    A distinct ``AGENTS.md`` in the current workdir is additive project
    context only when ``include_project_context`` is true. Gateway sessions
    keep this false so a changed tool cwd cannot silently inject a second
    instruction file; explicitly selected project context is routed by the
    task instead. Legacy ``NEYRA.md``, ``CLAUDE.md`` and Cursor rule files are
    not context sources in the Neyra runtime.

    ``SOUL.md`` remains an independent identity layer loaded only from the
    active Neyra home. Each loaded file is capped at 20,000 characters.
    """
    cwd_path = Path(cwd or os.getcwd()).resolve()
    home_path = get_neyra_home().resolve()
    sections = []

    central_core = _load_agents_md(home_path)
    if central_core:
        sections.append(central_core)

    if include_project_context and cwd_path != home_path:
        project_context = _load_agents_md(cwd_path)
        if project_context:
