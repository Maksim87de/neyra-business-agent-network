...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.neyra/models_dev_cache.json)
...
def _filter_official_catalog(data: Any) -> Dict[str, Any]:
    """Remove unrelated model families before they reach Neyra's local cache."""
    if not isinstance(data, dict):
        return {}
    excluded = tuple(
        "".join(parts)
        for parts in (("her", "mes"), ("kor", "ra"), ("гер", "мес"), ("кор", "ра"))
    )
    ...
    for provider_id, provider_data in data.items():
        ...
        if isinstance(models, dict):
            provider["models"] = {
                ...
                if not any(
                    term in (
                        str(model_id) + " " + json.dumps(model_data, ensure_ascii=False)
                    ).lower()
                    for term in excluded
                )
            }
        filtered[str(provider_id)] = provider
    return filtered
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Neyra ↔ models.dev
# ---------------------------------------------------------------------------

# Neyra provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Neyra (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_neyra_home() / "models_dev_cache.json"
    ...
    try:
        ...
        if cache_path.exists():
            with open(cache_path, encoding="utf-8") as f:
                return _filter_official_catalog(json.load(f))
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``neyra config refresh``, the
    ...
    try:
        ...
        data = _filter_official_catalog(response.json())
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Neyra provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from neyra_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Neyra agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Neyra provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Neyra ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Neyra or models.dev provider ID.  Tries exact match then
