...
from neyra_constants import display_neyra_home as _dhh_fn, PARTIAL_STREAM_STUB_ID
from neyra_logging import set_session_context
...
_UNBACKED_TOOL_RECEIPT_RE = re.compile(
    r"(?im)"
    r"(?:^|\n)\s*(?:tool\s*call|tool\s*result|function\s*call)\s*:"
    r"|(?:^|\n)\s*status\s*:\s*(?:completed|complete|success|succeeded)\b"
    r"|(?:^|\n)\s*(?:[✅✓]\s*)?(?:task|reminder|cron\s*job)\s+"
    r"(?:added|created|scheduled)\b"
)
_UNBACKED_TOOL_ARTIFACT_RE = re.compile(
    r"(?i)\b(?:post_id|job_id|cronjob|wall\.post|wall\.getById|vk\s*api|"
    r"vk\s*token|secret file|\.secrets|task added|reminder created)\b"
)
_UNBACKED_TOOL_ACTION_RE = re.compile(
    r"(?i)\b(?:created|saved|stored|published|posted|scheduled|sent|deleted|"
    r"updated|verified|wrote|создал[аи]?|создан[ао]?|сохранил[аи]?|"
    r"сохран[её]н[ао]?|записал[аи]?|опубликовал[аи]?|опубликован[ао]?|"
    r"отправил[аи]?|отправлен[ао]?|запланировал[аи]?|запланирован[ао]?|"
    r"проверил[аи]?|проверен[ао]?|добавил[аи]?)\b"
)
_UNBACKED_TOOL_EXECUTION_RESPONSE = (
    "I need to correct that: I did not execute any tools in this turn, so I "
    "cannot claim that I created, saved, scheduled, sent, or published "
    "anything. I can proceed only by running the relevant tool and then "
    "reporting the real result."
)


def _count_turn_tool_calls(messages: List[Dict[str, Any]], current_turn_user_idx: int) -> int:
    """Count structured assistant tool-call messages after the current user turn."""
    start = max(current_turn_user_idx + 1, 0)
    return sum(
...
def _looks_like_unbacked_tool_execution_claim(text: str) -> bool:
    """Detect assistant text that imitates a tool receipt without tool metadata."""
    if not isinstance(text, str) or not text.strip():
        return False
    if _UNBACKED_TOOL_RECEIPT_RE.search(text):
        return True
    return bool(
        _UNBACKED_TOOL_ARTIFACT_RE.search(text)
        and _UNBACKED_TOOL_ACTION_RE.search(text)
    )
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Neyra tool use: "
    ...
    return (
        ...
        f"context, but Neyra needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Neyra config, set `model.ollama_num_ctx: 65536` "
...
def _nous_entitlement_message(capability: str) -> str:
    try:
        from neyra_cli.nous_account import (
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    ...
    try:
        from neyra_cli.auth import NOUS_INFERENCE_AUTH_MODE_LEGACY
        from neyra_cli.nous_account import get_nous_portal_account_info
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if stored_prompt:
        ...
        try:
            from agent.runtime_observability import log_prompt_event
            log_prompt_event(
...
def run_conversation(
    ...
    # ``neyra logs --session <id>`` can filter a single conversation.
    from neyra_logging import set_session_context
    ...
    # User-outcome confirmation is independent of successful tool transport.
    # Integrations with explicit readback evidence may set this during a turn.
    agent._user_outcome_confirmed = False
    ...
    # are reused.  The system prompt is Neyra's territory; plugins
    ...
    # all run inside Codex). Default Neyra path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Neyra internals.
        #
        # Neyra invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Neyra tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if env_var_enabled("NEYRA_DUMP_REQUESTS"):
                ...
                if (
                    ...
                    from neyra_constants import display_neyra_home as _dhh_fn
                    _dhh = _dhh_fn()
                    _body_text = ""
                    try:
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: neyra auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `neyra doctor` for credential-chain diagnostics, or")
                    ...
                    from neyra_constants import display_neyra_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Neyra-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: neyra config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: neyra config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Neyra).  A 429 can
                ...
                # which Neyra' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Neyra' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`neyra",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `neyra auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `neyra model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: neyra auth add nous --type oauth", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: neyra setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        neyra fallback add   (interactive picker — same as `neyra model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a Neyra/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `neyra fallback add`."
                        )
                        return {
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                if (
                    _count_turn_tool_calls(messages, current_turn_user_idx) == 0
                    and _looks_like_unbacked_tool_execution_claim(final_response)
                ):
                    logger.warning(
                        "Blocked assistant final response that looked like an "
                        "unbacked tool execution claim: session=%s model=%s "
                        "provider=%s",
                    ...
                    agent._emit_status(
                        "⚠️ Model claimed tool execution without a tool call — "
                        "blocked unsafe receipt"
                    )
                    final_response = _UNBACKED_TOOL_EXECUTION_RESPONSE
                    _turn_exit_reason = "blocked_unbacked_tool_execution_claim"
                ...
                if not _turn_exit_reason.startswith("blocked_"):
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("NEYRA_KANBAN_TASK")
        if _kanban_task:
            try:
    ...
    try:
        from agent.runtime_observability import log_runtime_outcome_event
        log_runtime_outcome_event(
            outcome=_turn_exit_reason,
            confirmed=bool(getattr(agent, "_user_outcome_confirmed", False)),
            completed=bool(completed),
            failed=bool(failed),
            interrupted=bool(interrupted),
            guardrail_halted=agent._tool_guardrail_halt_decision is not None,
            tool_call_count=_count_turn_tool_calls(messages, current_turn_user_idx),
        ...
        logger.debug("user-outcome telemetry error: %s", _telemetry_err)
