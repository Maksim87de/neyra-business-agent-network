...
A pre-run snapshot of ``~/.neyra/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.neyra/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.neyra/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_neyra_home() / "skills" / ".curator_backups"


def _skills_dir() -> Path:
    return get_neyra_home() / "skills"


def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.neyra/cron/jobs.json``)."""
    return get_neyra_home() / "cron" / "jobs.json"
...
def is_enabled() -> bool:
    """Return whether the operator explicitly enabled curator snapshots."""
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.neyra/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.neyra/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.neyra/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.neyra/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `neyra curator rollback --list` to see available snapshots)",
            None,
        )
