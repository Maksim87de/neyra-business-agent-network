...
upstream prefix cache warm.  Neyra operator guidance documents the
system-prompt invariants and how the background-review fork inherits the
cached prompt verbatim.
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Neyra never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want NEYRA_HOME persona while keeping
    ...
    # Pointer to Neyra docs for user questions about Neyra itself.
    stable_parts.append(NEYRA_AGENT_HELP_GUIDANCE)
    ...
    # NEYRA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Neyra profile the agent is running
    # under so it doesn't conflate ~/.neyra/skills/ (default profile) with
    # ~/.neyra/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Neyra profile: default. Other profiles (if any) live "
            "under ~/.neyra/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Neyra profile: {active_profile}. This session reads "
            f"and writes ~/.neyra/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.neyra/skills/, ~/.neyra/plugins/, "
            f"~/.neyra/cron/, ~/.neyra/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the Neyra install
        ...
        context_files_prompt = _r.build_context_files_prompt(
            ...
            include_project_context=platform_key in {"", "cli"},
        )
    ...
    from neyra_time import now as _neyra_now
    now = _neyra_now()
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Neyra never rebuilds or reinjects parts of it
