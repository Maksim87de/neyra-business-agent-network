...
const LEGACY_STORAGE_KEY = "neyra-dashboard-theme";
const STORAGE_KEY = "neyra-dashboard-color-mode";

function normaliseThemeName(name: string | null | undefined): string {
  if (name && BUILTIN_THEMES[name]) return name;
  return "default";
}
 ...
 *  `_THEME_NAMED_ASSET_KEYS` in `neyra_cli/web_server.py`. */
...
const CUSTOM_CSS_STYLE_ID = "neyra-color-mode-custom-css";
...
function applyCustomCSS(css: string | undefined) {
  ...
  if (!el) {
    ...
    el.setAttribute("data-neyra-color-mode-css", "true");
...
function injectFontStylesheet(url: string | undefined) {
  ...
  link.rel = "stylesheet";
  link.href = url;
  link.setAttribute("data-neyra-color-mode-font", "true");
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  /** Name of the currently active color mode. */
  const [themeName, setThemeName] = useState<string>(() => {
    if (typeof window === "undefined") return "default";
    const stored =
      window.localStorage.getItem(STORAGE_KEY) ??
      window.localStorage.getItem(LEGACY_STORAGE_KEY);
    return normaliseThemeName(stored);
  });

  /** All selectable color modes. */
  ...
  // Resolve a color-mode name to a full DashboardTheme, falling back to dark.
  ...
  // Re-apply on every color-mode change.
  useEffect(() => {
    applyTheme(resolveTheme(themeName));
  }, [themeName, resolveTheme]);

  // Load server-side color-mode labels once on mount.
  useEffect(() => {
    let cancelled = false;
    api
      .getThemes()
      .then((resp) => {
        ...
        if (resp.themes?.length) {
          setAvailableThemes(
            resp.themes
              .filter((t) => BUILTIN_THEMES[t.name])
              .map((t) => ({
                name: t.name,
                label: t.label,
                description: t.description,
        ...
        const active = normaliseThemeName(resp.active);
        if (active !== themeName) {
          setThemeName(active);
  ...
  const setTheme = useCallback(
    (name: string) => {
      const next = normaliseThemeName(name);
      setThemeName(next);
