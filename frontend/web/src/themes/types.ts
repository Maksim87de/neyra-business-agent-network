/**
 * Dashboard color-mode model.
 *
 * Color modes customise three orthogonal layers:
 *
 ...
 *
 * Plus an optional `colorOverrides` escape hatch for modes that want to
 * pin specific shadcn tokens to exact values (e.g. a light mode that
...
export interface ThemePalette {
  ...
  warmGlow: string;
  /** Scalar multiplier (0–1.2) on the noise overlay. */
  noiseOpacity: number;
}
 ...
 *  viewport width. Color modes set this; plugins react via CSS vars /
 *  `[data-layout-variant="..."]` selectors. */
export type ThemeLayoutVariant = "standard" | "cockpit" | "tiled";

/** Named hero/background assets a color mode can populate. Each value is
 ...
 *  to render its hero render without coupling to the color-mode name. */
 ...
 *  strings — we don't parse them, so modes can use `clip-path`,
 ...
 *
 * The `themes` key is kept for API compatibility; Neyra currently exposes
 * only built-in dark/light modes.
 */
