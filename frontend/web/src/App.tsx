...
function buildNavItems(
  ...
  for (const manifest of manifests) {
    ...
    if (manifest.label === "Example" || manifest.tab.path === "/example") continue;

    const pluginItem: NavItem = {
      path: manifest.tab.path,
      label: pluginLabels[manifest.label] ?? manifest.label,
...
const SIDEBAR_COLLAPSED_KEY = "neyra-sidebar-collapsed";
...
export default function App() {
  ...
  const isTerminalChatRoute = normalizedPath === "/chat";
  const embeddedChat = isDashboardEmbeddedChatEnabled();
  // Neyra overlay: when NEYRA_DASHBOARD_CHAT=1, /chat is our bubble SSE chat
  // (rendered as a normal route, not the persistent PTY host) and takes
  // precedence over the upstream TUI chat.
  ...
  const builtinRoutes = useMemo(
    ...
    [embeddedChat, bubbleChat],
  );

  const builtinNav = useMemo(() => {
    const base = embeddedChat || bubbleChat
      ? [CHAT_NAV_ITEM, ...BUILTIN_NAV_REST]
      : BUILTIN_NAV_REST;
    return showTokenAnalytics
      ? base
      : base.filter((n) => n.path !== "/analytics");
  }, [embeddedChat, bubbleChat, showTokenAnalytics]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span
          className="select-none pointer-events-none text-xl font-semibold leading-none tracking-tight text-text-primary"
          style={{ marginLeft: 6 }}
        >
          {t.app.brand}
        </span>
          ...
          >
            ...
            >
              ...
              >
                ...
                <span className="select-none pointer-events-none text-2xl font-semibold leading-none tracking-tight text-text-primary">
                  {t.app.brand}
                </span>
              </div>
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="neyra-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="neyra-sidebar-plugin-nav-heading"
                  >
            ...
            >
              ...
              >
                ...
                <SidebarIconWithTooltip
                  collapsed={isDesktopCollapsed}
                  label={t.theme?.switchTheme ?? "Toggle color mode"}
                  tooltipWarmRef={tooltipWarmRef}
                >
              ...
              >
                ...
                {embeddedChat &&
                  ...
                  (pluginsLoading ? (
                    ...
                    <div
                      data-chat-active={isTerminalChatRoute ? "true" : "false"}
                      className={cn(
                        "min-h-0 min-w-0",
                        isTerminalChatRoute ? "flex flex-1 flex-col" : "hidden",
                      )}
                      aria-hidden={!isTerminalChatRoute}
                    >
                      <ChatPage isActive={isTerminalChatRoute} />
                    </div>
                  ))}
              </div>
