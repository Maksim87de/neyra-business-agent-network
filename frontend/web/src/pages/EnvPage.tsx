...
import { localizeEnvDescription } from "@/i18n/dynamic-ru";
...
const PROVIDER_GROUPS: { prefix: string; name: string; priority: number }[] = [
  ...
  { prefix: "NEYRA_QWEN_", name: "DashScope (Qwen)", priority: 2 },
...
function EnvVarRow({
      ...
      <div className="flex items-center justify-between gap-3 py-1.5 min-w-0 overflow-hidden text-text-secondary hover:text-foreground transition-colors">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs text-text-tertiary truncate hidden sm:block">
            {localizeEnvDescription(varKey, info.category, info.description)}
          </span>
        </div>
  ...
  return (
    <div className="grid gap-2 border border-border p-4 min-w-0 overflow-hidden">
      ...
      <p className="text-xs text-muted-foreground">{localizeEnvDescription(varKey, info.category, info.description)}</p>
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className="font-semibold text-sm tracking-wide">
            {group.name === "Другое" ? t.common.other : group.name}
          </span>
          {hasAnyConfigured && (
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col gap-1">
          <p className="text-sm text-muted-foreground">
            {t.env.description} <code>~/.neyra/.env</code>
          </p>
