/**
 * ChatPage — embeds `neyra --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (neyra_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { NEYRA_BASE_PATH, buildWsAuthParam } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${NEYRA_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__NEYRA_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `neyra dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Neyra TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[neyra-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__NEYRA_AUTH_REQUIRED__``) this
    ...
    ws.onclose = (ev) => {
      ...
      if (ev.code === 4403) {
        setBanner("Чат доступен только с локального адреса.");
        return;
      }
      if (ev.code === 1011) {
    ...
    // forwarding them into Neyra. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="neyra-chat-xterm-host min-h-0 min-w-0 flex-1"
          />

          <Button
            ghost
            onClick={handleCopyLast}
            title="Скопировать последний ответ ассистента в формате Markdown"
            aria-label="Скопировать последний ответ ассистента"
            className={cn(
              "absolute z-10",
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              ...
              <span className="hidden min-[400px]:inline tracking-wide">
                {copyState === "copied" ? "скопировано" : "скопировать последний ответ"}
              </span>
            </span>
          </Button>
        </div>
