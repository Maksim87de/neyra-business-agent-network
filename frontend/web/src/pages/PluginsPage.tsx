...
import { localizePluginDescription, localizeUiText } from "@/i18n/dynamic-ru";

/** Select value for built-in memory (`config` uses empty string). Never use `""` — UI Select maps empty value to an empty label. */
const MEMORY_PROVIDER_BUILTIN = "__neyra_memory_builtin__";
...
export default function PluginsPage() {
  ...
  const onInstall = async () => {
    ...
    try {
      ...
      showToast(`${r.plugin_name ?? id}: установлен`, "success");
      ...
      showToast(e instanceof Error ? e.message : "Не удалось установить", "error");
    } finally {
  ...
  const onRescan = async () => {
    ...
    try {
      ...
      showToast(e instanceof Error ? e.message : "Не удалось пересканировать", "error");
    } finally {
      setRescanBusy(false);
    }
  };
        ...
        {(hub?.orphan_dashboard_plugins?.length ?? 0) > 0 ? (
          ...
          <div className="flex flex-col gap-3 opacity-95">
            ...
            <ul className="flex flex-col gap-2 rounded border border-current/15 p-4">
              ...
              {hub!.orphan_dashboard_plugins.filter((m) => m.label !== "Example" && m.name !== "example").map((m) => (

                <li className="text-xs text-text-secondary" key={m.name}>


                  {(m.label === "Kanban" ? "Канбан" : m.label === "Achievements" ? "Достижения" : m.label ?? m.name)} — {localizePluginDescription(m.name, m.description || m.tab?.path)}
      ...
      <CardContent className="flex flex-col gap-4 px-6 py-4">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-3">
            ...
            <Badge tone="outline">
              {t.pluginsPage.sourceBadge}: {localizeUiText(row.source)}
            </Badge>

            <Badge tone="outline">v{row.version || "—"}</Badge>

            <Badge tone={badgeTone}>{row.runtime_status === "enabled" ? "включён" : row.runtime_status === "disabled" ? "отключён" : localizeUiText(row.runtime_status)}</Badge>
        ...
        {row.description ? (
          <p className="min-w-0 w-full text-xs tracking-[0.06em] text-text-secondary break-words">
            {localizePluginDescription(row.name, row.description)}
          </p>
        ) : null}
        ...
        {row.auth_required ? (
          <div className="grid gap-1.5">
            <span className="text-xs text-text-secondary">{t.pluginsPage.authRequiredHint}</span>
            <div className="flex items-center gap-2 border border-border px-3 py-2">
              <code className="min-w-0 flex-1 truncate text-xs">{row.auth_command}</code>
              <CopyButton text={row.auth_command} label="Копировать" copiedLabel="Скопировано ✓" />
            </div>
          </div>
        ) : null}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          void setRuntimeLoading(row.name, async () => {
            ...
            showToast(`${row.name}: удалён`, "success");
          });
        }}
        title={t.pluginsPage.removeConfirm}
        description={`Плагин «${row.name}» будет удалён из агента.`}
        destructive
