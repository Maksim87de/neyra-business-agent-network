...
import { localizeFieldLabel, localizeSchemaDescription } from "@/i18n/dynamic-ru";
...
function FieldHint({ schema, schemaKey }: { schema: Record<string, unknown>; schemaKey: string }) {
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      {description && <span className="text-xs text-text-secondary">{localizeSchemaDescription(schemaKey, description)}</span>}
    </div>
  );
}
...
export function AutoField({
  ...
  const label = localizeFieldLabel(rawLabel);
