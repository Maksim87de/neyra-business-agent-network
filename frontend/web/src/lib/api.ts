...
// (e.g. https://mission-control.tilos.com/neyra/). The Python backend
// injects ``window.__NEYRA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__NEYRA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const NEYRA_BASE_PATH = readBasePath();
const BASE = NEYRA_BASE_PATH;

/** Prefix a path with the dashboard base path (reverse-proxy aware). Used by
 *  raw fetch() callers (e.g. SSE chat) that bypass fetchJSON. */
...
declare global {
  interface Window {
    __NEYRA_SESSION_TOKEN__?: string;
    __NEYRA_BASE_PATH__?: string;
    ...
    __NEYRA_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Neyra-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__NEYRA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``neyra update``, ``neyra gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__NEYRA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__NEYRA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("neyra.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("neyra.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__NEYRA_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("neyra.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__NEYRA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Neyra dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``neyra_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__NEYRA_AUTH_REQUIRED__) {
...
export const api = {
  ...
  uploadChatAttachment: async (
    ...
    const result = await new Promise<{
      ...
      xhr.open("POST", `${BASE}/api/chat/attachments`);
      ...
      xhr.addEventListener("load", () => {
        ...
        const detail = xhr.response?.detail ?? xhr.response?.error ?? xhr.statusText;
        reject(new Error(`${xhr.status}: ${detail || "Не удалось загрузить файл"}`));
      });
  ...
  transcribeChatAttachment: (attachment: ChatAttachment) =>
    fetchJSON<{ transcript: string; provider: string }>(
      `/api/chat/attachments/${encodeURIComponent(attachment.sessionId)}/${encodeURIComponent(attachment.id)}/transcribe`,
      { method: "POST" },
    ),
  deleteChatAttachment: (attachment: ChatAttachment) =>
    fetchJSON<{ ok: boolean }>(
      `/api/chat/attachments/${encodeURIComponent(attachment.sessionId)}/${encodeURIComponent(attachment.id)}`,
      { method: "DELETE" },
    ),
  fetchChatAttachmentBlob: async (attachment: ChatAttachment): Promise<Blob> => {
    const headers = new Headers();
    const token = window.__NEYRA_SESSION_TOKEN__;
    if (token) setSessionHeader(headers, token);
    const response = await fetch(
      `${BASE}/api/chat/attachments/${encodeURIComponent(attachment.sessionId)}/${encodeURIComponent(attachment.id)}`,
  ...
  updateNeyra: () =>
    fetchJSON<ActionResponse>("/api/neyra/update", { method: "POST" }),
...
// ── Dashboard color-mode types ─────────────────────────────────────────

export interface DashboardThemeSummary {
  description: string;
  label: string;
  name: string;
  /** Full mode definition for API compatibility. */
